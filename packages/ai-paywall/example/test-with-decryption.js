...
const ENDPOINT = process.env.ENDPOINT || '/premium'; // Maps to /hidden/dog in registry
 ...
 */
function constructSealApproveCall(packageId, registryId, resourceId, accessPassId, policyIdBytes) {
  return (tx, id) => {
    tx.moveCall({
...
async function decryptWithSeal(
  ...
  try {
    ...
    log(`   ✅ Content decrypted (${decryptedData.length} bytes)`, colors.green);
...
async function step2_purchase_access_pass(challenge) {
  ...
  try {
    ...
    const { headers, accessPassId } = await client.payForAccess(`${SERVER_URL}${ENDPOINT}`);
...
async function step4_decrypt_content(encryptedBlob, resourceEntry, accessPassId, client) {
  ...
  try {
    ...
    log(`   ⚠️  Decryption requires signed SessionKey: ${error.message}`, colors.yellow);
...
async function main() {
  log('╔════════════════════════════════════════════════════════════╗', colors.blue);
  log('║   ai-paywall Complete Flow Test with Decryption           ║', colors.blue);
  log('╚════════════════════════════════════════════════════════════╝', colors.blue);
  log('');
  ...
  try {
    ...
    const { headers, accessPassId, client } = await step2_purchase_access_pass(challenge);
    ...
    if (contentResult.type === 'encrypted') {
      ...
      log(`   Resource Entry ID: ${REGISTERED_CONTENT.resourceEntryId}`, colors.cyan);
      log('');
      
      // Save encrypted blob
      const fs = require('fs');
      fs.writeFileSync('encrypted-content.bin', Buffer.from(contentResult.data));
      log('   ✅ Saved encrypted blob to: encrypted-content.bin', colors.green);
      
      // Attempt decryption with registered content data
      try {
        log('   Attempting decryption with registered content data...', colors.yellow);
        ...
        log(`   ⚠️  Decryption requires signed SessionKey: ${decryptError.message}`, colors.yellow);
        log('   See README_DECRYPTION.md for full decryption instructions', colors.yellow);
      }
