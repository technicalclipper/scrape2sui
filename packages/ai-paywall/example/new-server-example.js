 ...
 *   export RESOURCE_ENTRY_ID=0x...  # Optional: speeds up lookup (middleware queries registry if not provided)
 *   export RECEIVER_ADDRESS=0x...    # Your wallet address
 *   node example/new-server-example.js
 *
 * Note: ResourceEntry ID is optional. The middleware will automatically query
 * the registry on-chain to find resources by domain/resource. This means you
 * can register new content in registry-app and serve it immediately without
...
// After registering content in registry-app, you'll receive:
// - Domain (e.g., "www.demo1.com")
// - Resource path (e.g., "/hidden/dog")
// - ResourceEntry ID (optional - speeds up lookups)
// - Price, receiver address, etc.
const REGISTERED_RESOURCE = {
  ...
  // Optional: ResourceEntry ID from registry (optimization - middleware queries registry if not provided)
  resourceEntryId:
    process.env.RESOURCE_ENTRY_ID ||
    "0x44ace4be0c2ca4bf48bdb4f6a8069ff2beb73c3b33409035cffefa160ff40f5d", // Optional: speeds up lookup but not required
};
...
app.use(
  ...
  paywall({
    ...
    domain: REGISTERED_RESOURCE.domain, // Must match registry registration exactly
    resourceEntryId: REGISTERED_RESOURCE.resourceEntryId, // Optional: optimization cache
    // Note: If resourceEntryId is not provided, middleware will automatically
    // query the registry on-chain to find the ResourceEntry by domain/resource.
    // This allows you to add new content without changing code!
  })
);
...
app.get(REGISTERED_RESOURCE.resource, (req, res) => {
  ...
  if (req.paywall?.encryptedBlob) {
    ...
    console.log(
      `[Server] Serving encrypted blob (${req.paywall.encryptedBlob.byteLength} bytes)`
    );
    ...
    res.setHeader(
      "X-Seal-Policy",
      req.paywall.resourceEntry?.seal_policy || ""
    );
...
app.listen(PORT, () => {
  ...
  console.log(
    `🔒 Protected route: http://localhost:${PORT}${REGISTERED_RESOURCE.resource}`
  );
  ...
  console.log(
    `   const content = await client.access('http://localhost:${PORT}${REGISTERED_RESOURCE.resource}');`
  );
  console.log("");
});
