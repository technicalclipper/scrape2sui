...
async function step2_purchase_access_pass(challenge, nonce) {
  ...
  try {
    ...
    let gasOutput;
    try {
      gasOutput = execSync('sui client gas --json', { encoding: 'utf-8', stdio: 'pipe' });
      ...
      if (gasCoins.length > 0 && !gasCoins[0].balance) {
        log(`   ⚠️  Debug: First coin structure: ${JSON.stringify(gasCoins[0]).substring(0, 200)}`, colors.yellow);
      }
    } catch (error) {
    ...
    const getCoinBalance = (coin) => {
      ...
      if (coin.data && coin.data.content && coin.data.content.fields && coin.data.content.fields.balance) {
    ...
    for (const coin of gasCoins) {
      ...
      if (!coinId) {
        log(`   ⚠️  Skipping coin with unknown structure: ${JSON.stringify(coin).substring(0, 100)}`, colors.yellow);
        continue;
      }
        ...
        try {
          ...
          if (fetchedBalance > BigInt(0)) {
            ...
            if (fetchedBalance > largestBalance) {
              ...
              largestCoin = { coinObjectId: coinId, balance: fetchedBalance.toString() };
            }
            log(`   Coin ${coinId}: ${fetchedBalance.toString()} MIST (${Number(fetchedBalance) / 1_000_000_000} SUI)`, colors.gray);
            continue;
          }
        } catch (e) {
      ...
      log(`   Coin ${coinId}: ${balance.toString()} MIST (${Number(balance) / 1_000_000_000} SUI)`, colors.gray);
    }
    
    log(`   Total balance: ${totalBalance.toString()} MIST (${Number(totalBalance) / 1_000_000_000} SUI)`, colors.cyan);
    ...
    for (const coin of gasCoins) {
      ...
      if (balance >= requiredBalance) {
        sourceCoinId = coinId;
        log(`   ✅ Selected coin: ${sourceCoinId} (balance: ${balance.toString()} MIST)`, colors.green);
        break;
      }
    }
    ...
    if (!sourceCoinId) {
      if (totalBalance < requiredBalance) {
        throw new Error(`Insufficient total balance. Need at least ${priceMist} MIST (${challenge.price} SUI). Total: ${Number(totalBalance) / 1_000_000_000} SUI`);
      }
      ...
      log(`   ⚠️  No single coin has ${priceMist} MIST, using largest coin: ${sourceCoinId} (balance: ${largestBalance.toString()} MIST)`, colors.yellow);
      log(`   Total balance is sufficient, will split ${priceMist} MIST from largest coin.`, colors.cyan);
    }
    ...
    try {
        ...
        if (splitResult.effects && splitResult.effects.created) {
          for (const created of splitResult.effects.created) {
            if (created.reference && created.reference.objectId) {
              ...
              try {
                const coinCheck = execSync(`sui client object ${created.reference.objectId} --json`, {
                  encoding: 'utf-8',
                  stdio: 'pipe'
                });
                const coinObj = JSON.parse(coinCheck);
                if (coinObj.data && coinObj.data.type && coinObj.data.type.includes('Coin')) {
      ...
      if (!paymentCoinId) {
        throw new Error('Could not extract payment coin ID from split transaction. Please check your wallet.');
      }
    ...
    try {
      ...
      const resourceHex = Buffer.from(challenge.resource, 'utf8').toString('hex');
      ...
      const purchaseCmd = `sui client call ` +
      ...
      try {
        purchaseOutput = execSync(purchaseCmd, { encoding: 'utf-8', stdio: 'pipe' });
      } catch (error) {
        ...
        const purchaseCmd2 = `sui client call ` +
          `--package ${CONTRACT.packageId} ` +
          `--module paywall ` +
          `--function purchase_pass ` +
        ...
        try {
          purchaseOutput = execSync(purchaseCmd2, { encoding: 'utf-8', stdio: 'pipe' });
        } catch (error2) {
      ...
      if (!accessPassId && purchaseResult.effects && purchaseResult.effects.created) {
        for (const created of purchaseResult.effects.created) {
          if (created.reference && created.reference.objectId) {
            // Verify it's an AccessPass by checking type
            try {
              const objCheck = execSync(`sui client object ${created.reference.objectId} --json`, {
                encoding: 'utf-8',
                stdio: 'pipe'
              });
              const obj = JSON.parse(objCheck);
              if (obj.data && obj.data.type && obj.data.type.includes('AccessPass')) {
      ...
      if (!accessPassId) {
        try {
          ...
          for (const line of lines) {
            if (line.includes('AccessPass') || line.includes('Created Objects')) {
              const match = line.match(/0x[a-fA-F0-9]{64}/);
              if (match) {
                accessPassId = match[0];
                // Verify it's actually an AccessPass
                try {
                  const verify = execSync(`sui client object ${accessPassId} --json`, {
                    encoding: 'utf-8',
                    stdio: 'pipe'
                  });
                  const verifyObj = JSON.parse(verify);
                  if (verifyObj.data && verifyObj.data.type && verifyObj.data.type.includes('AccessPass')) {
                    break;
                  }
                } catch (e) {
                  accessPassId = null;
      ...
      if (!accessPassId) {
        log('   ⚠️  Could not automatically extract AccessPass ID from transaction', colors.yellow);
        log('   Transaction output:', colors.cyan);
        console.log(purchaseOutput);
        log('');
        log('   Please manually extract AccessPass ID from "Created Objects" section', colors.yellow);
        ...
        return new Promise((resolve) => {
          rl.question('📝 Enter AccessPass ID (from transaction output): ', (input) => {
            const id = input.trim();
            rl.close();
            if (!id || !id.startsWith('0x') || id.length !== 66) {
              throw new Error('Invalid AccessPass ID format. Must start with 0x and be 66 characters.');
            }
...
async function step3_get_access_pass_details(accessPassId) {
  ...
  try {
    ...
    const fields = object.data?.content?.fields || object.content?.fields || object.data?.fields || {};
...
async function step4_request_content(owner, accessPassId) {
  ...
  if (response.status === 200) {
    ...
    console.log(JSON.stringify(response.body, null, 2));
    log('\nNote: Signature verification might fail with placeholder signature.', colors.yellow);
...
async function step5_consume_pass(accessPassId, expectedRemaining) {
  ...
  try {
    ...
    const consumeCmd = `sui client call ` +
      `--package ${CONTRACT.packageId} ` +
      `--module paywall ` +
    ...
    let consumeOutput;
    try {
      consumeOutput = execSync(consumeCmd, { encoding: 'utf-8', stdio: 'pipe' });
    } catch (error) {
    ...
    if (newRemaining === expectedRemaining - 1) {
      ...
      log(`⚠️  Expected remaining to be ${expectedRemaining - 1}, but got ${newRemaining}`, colors.yellow);
      return newRemaining;
    }
  } catch (error) {
...
async function main() {
  log('╔════════════════════════════════════════════════════════════╗', colors.blue);
  log('║      ai-paywall Complete Flow Test                        ║', colors.blue);
  log('╚════════════════════════════════════════════════════════════╝', colors.blue);
  log('');
  ...
  log('  6. Consume pass (decrement remaining uses) → Verify decreased', colors.yellow);
  log('');
  ...
  try {
    ...
    const { owner, remaining } = await step3_get_access_pass_details(accessPassId);
    ...
    if (success && newRemaining === remaining - 1) {
      ...
      log('⚠️  Test Complete - Most steps passed (remaining uses may need verification)', colors.yellow);
    } else {
      log('⚠️  Test Complete - Most steps passed (signature verification may need real signing)', colors.yellow);
    }
    log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━', colors.blue);
    console.log();
